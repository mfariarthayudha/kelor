...
export const getDocumentApi = async (request: Request, response: Response) => {
  try {
    let draw = request.body.draw;
    let start: number = +request.body.start;
    let length: number = +request.body.length;
    let order_data = request.body.order;
    var search_value: string = request.body.search["value"];

    const total_records = await knex("document_results")
      .count("document_result_id as jumlah")
      .then((res) => {
        return res[0].jumlah;
      });

    const total_records_with_filter = await knex("document_types")
    ...
    if (order_data !== undefined) {
      ...
      var column_name: string = request.body.columns[column_index]["data"];
      var column_sort_order: string = request.body.order[0]["dir"];

      query.orderBy(column_name, column_sort_order);
    }
    const data_arr = await query
      .select(
